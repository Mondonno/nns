...
\Large A Comprehensive Educational Deep Learning Framework}
...
\begin{itemize}
    ...
    \item \textbf{Convolution2D}: 2D convolutional layers for spatial data
    \item \textbf{MaxPool2D}: Max pooling for dimensionality reduction
    \item \textbf{Flatten}: Reshaping between convolutional and dense layers
\end{itemize}
...
\begin{equation}
(\mathbf{X} * \mathbf{K})_{i,j} = \sum_{m=0}^{M-1} \sum_{n=0}^{N-1} \mathbf{X}_{i+m,j+n} \cdot \mathbf{K}_{m,n}
\end{equation}
...
\node[box, below of=check, yshift=-0.5cm, xshift=-3cm] (pass) {Pass ✓};
\node[box, below of=check, yshift=-0.5cm, xshift=3cm] (fail) {Fail ✗};
...
\node[phase] (p5) at (3.5,-3) {\textbf{Phase 5}\\Conv Layers};
\node[phase] (p6) at (7,-3) {\textbf{Phase 6}\\Verification};
...
\subsubsection{Phase 5: Convolutional Neural Networks}

Extended to handle spatial data:
\begin{itemize}
    \item 2D convolution operations
    \item Convolutional layers with kernels
    \item Max pooling layers
    \item Flatten layers for dimensionality transitions
\end{itemize}

\textbf{Key Challenge}: Implementing efficient convolution operations and computing gradients for convolutional layers.

\subsubsection{Phase 6: Verification and Testing}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Component} & \textbf{Phase 2} & \textbf{Phase 4} & \textbf{Phase 6} \\ \midrule
Functions & 3 & 5 & 7 \\
Layers & 1 & 2 & 5 \\
Optimizers & 0 & 3 & 3 \\
Tests & 0 & 2 & 8 \\
...
\centering
\begin{tikzpicture}[
    conv/.style={rectangle, draw, fill=red!20, minimum width=1.5cm, minimum height=2cm},
    pool/.style={rectangle, draw, fill=green!20, minimum width=1.2cm, minimum height=1.5cm},
    dense/.style={rectangle, draw, fill=blue!20, minimum width=1cm, minimum height=3cm},
]

\node[conv] (conv1) at (0,0) {Conv 3×3};
\node[pool, right=0.5cm of conv1] (pool1) {MaxPool};
\node[conv, right=0.5cm of pool1] (conv2) {Conv 3×3};
\node[pool, right=0.5cm of conv2] (pool2) {MaxPool};
\node[dense, right=0.5cm of pool2] (flatten) {Flatten};
\node[dense, right=0.5cm of flatten] (fc) {Dense};
...
\node[below=0.2cm of conv1] {\scriptsize 28×28×1};
\node[below=0.2cm of pool1] {\scriptsize 14×14×32};
\node[below=0.2cm of conv2] {\scriptsize 14×14×32};
\node[below=0.2cm of pool2] {\scriptsize 7×7×64};
\node[below=0.2cm of fc] {\scriptsize 10};
...
\begin{itemize}
    \item \textbf{Forward Pass}: $O(k^2 \times C_{in} \times C_{out} \times H_{out} \times W_{out})$
    \item \textbf{Memory}: $O(k^2 \times C_{in} \times C_{out})$ for kernels
\end{itemize}
...
model = Sequential([
    Convolution2D(out_channels=32, kernel_size=(3,3)),
    MaxPool2D(pool_size=(2,2)),
    Convolution2D(out_channels=64, kernel_size=(3,3)),
    MaxPool2D(pool_size=(2,2)),
    Flatten(),
    Dense(128, 10, RectifiedLinearFunction())
], MSEFunction(), learning_rate)
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item He, K., Zhang, X., Ren, S., \& Sun, J. (2015). Delving deep into rectifiers: Surpassing human-level performance on ImageNet classification. \textit{ICCV}.
\end{enumerate}
...
\textit{This document and the associated codebase represent a comprehensive educational resource for understanding neural networks from first principles. All code is available under the MIT License.}\\[0.2cm]
